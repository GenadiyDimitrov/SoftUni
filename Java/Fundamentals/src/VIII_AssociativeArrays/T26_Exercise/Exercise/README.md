...
[Judge.](https://judge.softuni.org/Contests/1312)

## Count Chars in a String

Write a program that **counts all characters** in a string **except
space (' ')**.
...
of strings, each on a new line. Every **odd** line on the console
represents a **resource** (e.g., Gold, Silver, Copper, etc.) and every
...
Write a program which keeps the information about **products** and their
**prices**. Each product has a **name**, a **price,** and its
**quantity**. If the product **doesn't exist** yet, **add** it with its
**starting quantity**.

If you receive a product that **already exists,** **increases** its
...
Write a program that validates parking for an online service. Users can
...
Write a program that keeps the information about **students** and
**their grades**.
...
process and requires quite a bit of farming. The possible **items** are:
...
In the end, print the **remaining** **shards, fragments, and motes** in
the format:
...
You will receive company names and an employees' id until you receive
the "**End**" command. **Add** **each employee** to the given company.
Keep in mind that a **company cannot have two employees with the**
**same id**.
...
If you receive a **"force\_user -\> force\_side"**:
  ...
  - If there is no such **force user** on any **force side** -\> add the
    **force user** to the corresponding **force side**.

  - **If there is no such force user** and no such **force side -\>
    create a new force side and add** the **force user** to the
    corresponding side.
...
<td><p><strong>Although Ivan Ivanov doesn't have a profile, we registered him and added him to the Lighter side.</strong></p>
...
Judge statistics on the last Programming Fundamentals exam were not
...
the final results and statistics about each language in which the
...
The participant's points will always be a **valid integer in the range
\[0-100\].**
